...
const MyCamera = ({ isInquiring, ...props }: any) => {
	...
	const camPosition = { x: 6.5, y: 3.5, z: 6.5 };
	const ref = useRef<THREE.PerspectiveCamera>(null);

	useFrame((state, delta) => {
		if (!ref.current) return;
		if (isInquiring) {
			ref.current.position.x = approachValue(ref.current.position.x, interactPersonView.x, 0.03);
			ref.current.position.y = approachValue(ref.current.position.y, interactPersonView.y, 0.03);
			ref.current.position.z = approachValue(ref.current.position.z, interactPersonView.z, 0.03);
